...
void HelmoltzProblem<dim>::assemble_system()
{
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
            ...
            for (unsigned int j = 0; j < dofs_per_cell; ++j)
            {
                    ...
                    ++q_point)
                {
                    ...
                    if ((component_i == component_j) && (component_i < 2))
                    {
                        cell_matrix(i, j)
                            +=
                    ...
                    {
                        cell_rhs(i) -= fe_values.JxW(q_point) * fe_values.shape_value(j, q_point) * fe_values.shape_value(i, q_point);
                    }
                }
            }
        }
    ...
    VectorTools::interpolate_boundary_values(dof_handler,
        0,
        ZeroFunction<dim>(dim + INCLUDE_TOTAL_CURRENT),
        boundary_values);
...
template <int dim>
void HelmoltzProblem<dim>::set_main_equation_index(DynamicSparsityPattern& dsp)
{
    QGauss<dim>  quadrature_formula(2);
    ...
    for (; cell != endc; ++cell)
    {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
            if (main_equation_index != local_dof_indices[i])
                ...
                system_rhs(local_dof_indices[i]) = TOTAL_CURRENT * getAreaOfTotalCurrent();
        }
    }
}
